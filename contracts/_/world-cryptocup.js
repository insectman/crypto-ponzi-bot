...
// const buyMaxLimit = 0.0103;

/*
(async () => {
  // const tkn = await Token.findOne({ name: 'cryptosports', tokenId: 0 });
  const tkn = await Token.findOneAndUpdate({ name: 'cryptosports', tokenId: 82 },
  { formattedPrice: 9999999 });
  console.log(tkn);
})();
...
const getTokenData = async (params) => {
  ...
  const tokenData = await contract.getAthlete(tokenId);
